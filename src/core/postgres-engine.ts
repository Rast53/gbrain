...
export class PostgresEngine implements BrainEngine {
  ...
  async getLinks(slug: string, opts?: { sourceId?: string; sourceIds?: string[] }): Promise<Link[]> {
    ...
    // Live-page visibility: both endpoints (f, t) must be undeleted so a
    // soft-deleted page cannot leak through the link API / portal. The origin
    // LEFT JOIN is intentionally unfiltered — origin_slug is provenance, not a
    // graph node. Pinned by test/links-deleted-at-filter.test.ts.
    return await this.withScopedReadTransaction(opts?.sourceIds, opts?.sourceId, async (tx) => {
  ...
  async getBacklinks(slug: string, opts?: { sourceId?: string; sourceIds?: string[] }): Promise<Link[]> {
    ...
    //   1. RLS scope binding (opt-in via GBRAIN_RLS_SCOPE_BINDING)
    //   2. App-layer source filter (#2200 federated)
    // Live-page visibility mirrors getLinks: filter f + t, leave origin JOIN
    // unfiltered. Pinned by test/links-deleted-at-filter.test.ts.
    return await this.withScopedReadTransaction(opts?.sourceIds, opts?.sourceId, async (tx) => {
  ...
  async traverseGraph(
    ...
    const aggScope = useSourceIds
      ...
      : opts?.sourceId
        ...
        : sql``;
    // Live-page visibility: seed, neighbor, and aggregated far-endpoint must
    // all be undeleted so a soft-deleted page cannot appear as a BFS node or
    // in the per-node links[] array. Pinned by test/links-deleted-at-filter.test.ts.
    const liveSeed = sql`AND p.deleted_at IS NULL`;
    const liveNeighbor = sql`AND p2.deleted_at IS NULL`;
    const liveAgg = sql`AND p3.deleted_at IS NULL`;
    ...
    const rows = await sql`
      WITH RECURSIVE graph AS (
        ...
        FROM pages p WHERE p.slug = ${slug} ${seedScope} ${liveSeed}
      ...
      SELECT DISTINCT g.slug, g.title, g.type, g.depth,
        coalesce(
          ...
          (SELECT jsonb_agg(DISTINCT jsonb_build_object('to_slug', p3.slug, 'link_type', l2.link_type))
           ...
           WHERE l2.from_page_id = g.id ${aggScope} ${liveAgg}),
          '[]'::jsonb
        ) as links
      FROM graph g
  ...
  async traversePaths(
    ...
    const ptScope = useSourceIds
      ...
      : opts?.sourceId
        ...
        : sql``;
    // Live-page visibility: seed + neighbor (and both endpoints on the 'both'
    // emit) must be undeleted. Mirrors traverseGraph. Pinned by
    // test/links-deleted-at-filter.test.ts.
    const liveSeed = sql`AND p.deleted_at IS NULL`;
    const liveNeighbor = sql`AND p2.deleted_at IS NULL`;
    const liveFrom = sql`AND pf.deleted_at IS NULL`;
    const liveTo = sql`AND pt.deleted_at IS NULL`;
