...
- `test/links-deleted-at-filter.test.ts` — soft-deleted pages disappear from `getLinks`/`getBacklinks` (federated, scoped, unscoped) and `traverseGraph`/`traversePaths`; `restore_page` brings them back. Origin LEFT JOIN stays unfiltered. Negative: `getPage({includeDeleted:true})` / `listPages` recovery window, unscoped cross-source live edges (reconcileLinks / back-link validators), and `purgeDeletedPages` stay unchanged.
